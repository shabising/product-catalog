...
export default function ProductDetail({ productId, onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-start justify-center overflow-y-auto p-6"
      onClick={onClose}
    >
      <div
          ...
          <div>
            ...
            <div className="px-5 pb-5">
              ...
              <div className="flex gap-2 flex-wrap text-xs">
                ...
                <span className="text-gray-400">{product.shippingInformation}</span>
                <span className="text-gray-400">{product.returnPolicy}</span>
              </div>
            </div>
          </div>
        ) : (
          <p className="text-center text-gray-400 py-20">Product not found.</p>
        )}
      </div>
    </div>
  );
